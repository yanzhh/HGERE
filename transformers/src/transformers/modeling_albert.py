...
class AlbertForSpanNER(AlbertPreTrainedModel):
	def __init__(self, config):
		super().__init__(config)
		self.max_seq_length = config.max_seq_length
		self.num_labels = config.num_labels
...
#----------------------------------------------
class AlbertForSpanMarkerNerPruner(AlbertPreTrainedModel):
	def __init__(self, config, args):
		...
		if self.biaf_span:
			...
			if self.args.extra_repr=='attn':
				self.extra_encoder = AttnSpanRepr(input_dim=config.hidden_size,proj_dim=args.rank, output_dim=args.span_size, dropout=config.hidden_dropout_prob)
	...
	def forward(
		...
		hidden_states = outputs[0]                                      # bs * tot_seq_len * dh
		if self.onedropout:
			hidden_states = self.dropout(hidden_states)
		...
		e1_hidden_states = hidden_states[:, seq_len:seq_len+ent_len]                # marker  bs * ent_len(max_pair_length) * dh 
		e2_hidden_states = hidden_states[:, seq_len+ent_len: ]
...
class AlBertForBaselines(AlbertPreTrainedModel):
	def __init__(self, config, args=None):
		...
		if self.args.baseline=='firstorder':
			...
			self.ner_cls = CatEncoder(input_dims=[sub_dim, obj_dim], output_dim=self.num_ner_labels)
	...
	def forward(
		...
		if self.args.baseline=='firstorder':
			...
			ner_prediction_scores = self.ner_cls(sub_reprs, uni_obj_reprs)                 # n_ent x max_ent_num x n_ent_label

			# rel_reprs = self.rel_encoder(sub_reprs.unsqueeze(-2).expand(obj_reprs.shape), obj_reprs)        # n_ent x max_ent x max_ent x dr
...
class AlbertForHyperGNN(AlbertPreTrainedModel):
	...
	def forward(
		...
		# return sub_reprs_list, obj_reprs_list, uni_obj_reprs_list        
		# rel_reprs = self.rel_encoder(sub_reprs.unsqueeze(-2).expand(obj_reprs.shape), obj_reprs)
...
class AlbertForPlmarker(AlbertPreTrainedModel):
	def __init__(self, config, args=None):
		super().__init__(config)
		self.max_seq_length = config.max_seq_length
		self.num_labels = config.num_labels
