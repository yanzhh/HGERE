...
CUDA_VISIBLE_DEVICES=$GPU_ID  python  run_hgnn.py  --model_type hyper  \
    ...
    --num_train_epochs $epoch --eval_epochs 3 \
    ...
    --seed $seed   \
    --ner_prediction_dir  saves/scier/pruner/biafencoder-spanlen12-rank768-hid768-span256-entnum3-16-lr2e-5-epochs8/scier_scibert-44 \
    --train_file ent_pred_2025-02-08_train.json \
    --dev_file ent_pred_2025-02-08_dev.json \
    --test_file ent_pred_2025-02-08_test.json  \
    --output_dir saves/scier/HGERE/scibert/$factor/facenc$facenc-seq$seq-mem$memdim-iter$iter-layernorm+_attnself/ent$entdim-rel$reldim-lr$lr-$lr1-bs$bs-ep$epoch-eps$eps/Hyper_scier_scibert-$seed  --overwrite_output_dir  \
