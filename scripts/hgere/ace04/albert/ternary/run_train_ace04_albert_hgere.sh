...
for lr1 in 1e-4; do
for seq in 384; do
for entdim in 512; do
for reldim in 512; do 
for memdim in 512; do
for facenc in biaf; do
for factor in ternary; do
for iter in 3; do
for eps in 1e-8; do
CUDA_VISIBLE_DEVICES=$GPU_ID  python  run_hgnn.py  --model_type alberthyper  \
    --model_name_or_path  pretrained_models/albert-xxlarge-v1  --do_lower_case  \
    ...
    --output_dir saves/HGERE/ace04re_models/albert/$factor/facenc$facenc-seq$seq-mem$memdim-iter$iter-eps$eps-layernorm+_attnself/ent$entdim-rel$reldim-lr$lr-$lr1-bs$bs-ep$epoch/Hyper_ace04_albert-fold$fold  --overwrite_output_dir  \
