# SciERC with ModernBERT + rule-based pre-filter + focal loss
GPU_ID=0
...
NICK_NAME=modernbert-focal-prefilter-${lr}-max_seq_length265
...
CUDA_VISIBLE_DEVICES=$GPU_ID  python3  run_pruner.py  \
    ...
    --overwrite_model_dir  \
    --output_results \
    --model_type modernbertspanmarkerpruner  \
    --base_model_name_or_path  pretrained_models/modernbert_base  \
    --learning_rate $lr  \
    --num_train_epochs 8  \
    --eval_epochs 1   \
