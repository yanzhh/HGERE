...
for lr in 1e-5; do
#--do_train \
OUTPUT_DIR=/home/groups/gsap/gsap-ere/models/pruner/default/
##$RUN_NAME/${DATE_MODEL}
echo $OUTPUT_DIR
CUDA_VISIBLE_DEVICES=$GPU_ID  python3  run_pruner.py  \
    ...
    --seed $seed \
    --data_dir $DATADIR \
    --train_file ${DATASET_DATE}_train.jsonl \
    --dev_file ${DATASET_DATE}_dev.jsonl \
    --test_file ${DATASET_DATE}_test.jsonl  \
    --output_dir ${OUTPUT_DIR} \
    --overwrite_output_dir  \
    --output_results \
    --model_type bertspanmarkerpruner  \
    --model_name_or_path  /home/groups/gsap/gsap-ere/models/pretrained_models/scibert_scivocab_uncased  \
    --do_lower_case  \
    --learning_rate $lr  \
