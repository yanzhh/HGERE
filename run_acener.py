...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            ner_values, ner_preds = torch.max(ner_logits, dim=-1)                           # max values and max indices
            ...
            for i in range(len(indexs)):
                ...
                for j in range(len(m2s)):
                    ...
                    if ner_label!='NIL':
                        scores[(index[0], index[1])][(obj[0], obj[1])] = (float(ner_values[i,j]), ner_label)        # scores record each sentence each entity prediction results: (probability, predicted label)
