"""
Pruner threshold analysis — selects best threshold or top-K config for the pruner output.

Usage
-----
    uv run gsapere-tune-pruner \\
...
ROOT = Path(__file__).resolve().parents[3]  # …/HGERE


def _count_total_candidates(docs: list) -> int:
    """Total number of spans scored by the pruner (before any threshold)."""
    return sum(len(sent) for doc in docs for sent in doc["predicted_ner_proba"])
...
def _overall_table(
    ...
    rows = [
        ...
        *(
            [["target_recall", _fmt(target_recall), _fmt(target_recall)]]
            if show_target
            else []
        ),
        [
            "precision",
            _fmt(thresh_metrics["precision"]),
            _fmt(topk_metrics["precision"]),
        ],
        ["f1", _fmt(thresh_metrics["f1"]), _fmt(topk_metrics["f1"])],
        [
            "false_positive_share",
            _fmt(thresh_metrics["false_positive_share"]),
            _fmt(topk_metrics["false_positive_share"]),
        ],
        ...
        [
            "TN/(FP+TN)",
            _fmt(_tn_rate(thresh_metrics, tn_thresh)),
            _fmt(_tn_rate(topk_metrics, tn_topk)),
        ],
    ...
    return tabulate(
        rows, headers=["metric", "score threshold", "top-K"], tablefmt="github"
    )
...
def main() -> None:
    ...
    best_threshold, thresh_dev_metrics = find_threshold_for_recall(
        dev_docs, target_recall=target_recall
    )
    print(
        f"  Best threshold: {best_threshold:.6f}  (recall={thresh_dev_metrics['recall']:.4f})"
    )
    ...
    best_topk_params, all_topk = optimize_topk_params(
        dev_docs, target_recall=target_recall
    )
    ...
    # ------------------------------------------------------------------
    thresh_metrics_dev = compute_metrics_at_threshold(
        dev_docs, best_threshold, entity_types=entity_types
    )
    topk_metrics_dev = compute_metrics_topk(
        dev_docs, lam, lmin, lmax, entity_types=entity_types
    )

    overall_tbl_dev = _overall_table(
        thresh_metrics_dev,
        best_threshold,
        topk_metrics_dev,
        n_dev_candidates,
        upper_bound_recall=dev_upper_bound,
        target_recall=target_recall,
        show_target=True,
    )
    per_type_tbl_dev = _per_type_table(
        thresh_metrics_dev["per_type"], topk_metrics_dev["per_type"], entity_types
    )
    ...
    if train_path:
        ...
        thresh_metrics_train = compute_metrics_at_threshold(
            train_docs, best_threshold, entity_types=entity_types
        )
        topk_metrics_train = compute_metrics_topk(
            train_docs, lam, lmin, lmax, entity_types=entity_types
        )
        ...
        train_upper_bound = (
            train_n_gold_in_pool / train_n_gold if train_n_gold > 0 else 1.0
        )

        overall_tbl_train = _overall_table(
            thresh_metrics_train,
            best_threshold,
            topk_metrics_train,
            n_train_candidates,
            upper_bound_recall=train_upper_bound,
            target_recall=target_recall,
            show_target=False,
        )
        per_type_tbl_train = _per_type_table(
            thresh_metrics_train["per_type"],
            topk_metrics_train["per_type"],
            entity_types,
        )
    ...
    if test_path:
        ...
        thresh_metrics_test = compute_metrics_at_threshold(
            eval_docs, best_threshold, entity_types=entity_types
        )
        topk_metrics_test = compute_metrics_topk(
            eval_docs, lam, lmin, lmax, entity_types=entity_types
        )
        ...
        overall_tbl_test = _overall_table(
            thresh_metrics_test,
            best_threshold,
            topk_metrics_test,
            n_eval_candidates,
            upper_bound_recall=eval_upper_bound,
            target_recall=target_recall,
            show_target=False,
        )
        per_type_tbl_test = _per_type_table(
            thresh_metrics_test["per_type"], topk_metrics_test["per_type"], entity_types
        )
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            f"| **Upper-bound recall (dev)** | {dev_upper_bound:.4f} "
        ...
        if test_path:
            f.write(
                f"| **Upper-bound recall (test)** | {eval_upper_bound:.4f} "
    ...
    if thresh_meets and topk_meets:
        best_method = (
            "threshold"
            if thresh_metrics_dev["false_positive_share"]
            <= topk_metrics_dev["false_positive_share"]
            else "topk"
        )
    elif thresh_meets:
    ...
    if best_method == "threshold":
        best_config = {"method": "threshold", "value": best_threshold}
    else:
        best_config = {"method": "topk", "ratio": lam, "min": lmin, "max": lmax}
    ...
    def _add_split(split, pred_file, thresh_m, topk_m, n_cands, ub_recall):
        overall_rows.append(
            _flatten_overall(
                ...
                topk_ratio=None,
                min_mentions_num=None,
                max_mentions_num=None,
        ...
        overall_rows.append(
            _flatten_overall(
                ...
                threshold=None,
                topk_ratio=lam,
                min_mentions_num=lmin,
                max_mentions_num=lmax,
                upper_bound_recall=ub_recall,
            )
        )
        per_type_rows.extend(
            _flatten_per_type(
                thresh_m["per_type"], entity_types, split, pred_file, "threshold"
            )
        )
        per_type_rows.extend(
            _flatten_per_type(
                topk_m["per_type"], entity_types, split, pred_file, "topk"
            )
        )
