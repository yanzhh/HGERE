"""CLI entry point: gsapere-train-pruner

``gsapere-train-pruner`` (``main()``) accepts two modes:

1. **YAML config** — single positional argument::

       uv run gsapere-train-pruner configs/train/gsap/train_gsap.yaml
...
:class:`~gsapere.pruner.config.PrunerTrainConfig` (the Pydantic model),
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    config = load_config_from_argv(
        ...
        description=(
            ...
            "(gsapere-train-pruner config.yaml) or supply all parameters directly as "
