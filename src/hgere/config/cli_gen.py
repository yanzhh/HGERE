...
- **Override-friendly**: Every flag has ``default=None`` in argparse so that
  only *explicitly supplied* CLI values override a config file.  Pydantic
  supplies all real defaults.
...
def build_argparser(
    ...
    Top-level fields and nested fields are separated into labelled argument
    groups in the ``--help`` output.  Every flag defaults to ``None`` so that
    only *explicitly provided* CLI values override a config file.

    A ``--config`` flag pointing to a YAML file is always prepended.
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Path to a YAML config file. CLI flags override individual values.",
    )
...
def namespace_to_nested_dict(namespace: argparse.Namespace) -> dict[str, Any]:
    ...
    for flat_key, value in vars(namespace).items():
        if flat_key == "config" or value is None:
            continue
...
def apply_config_and_cli(
    ...
    """Merge a YAML config file and CLI overrides, then validate with *model_cls*.

    Priority (highest first): CLI flags > config file > Pydantic defaults.
    ...
    config_loader:
        Optional callable ``(path: str) -> dict`` for loading the config file.
    ...
    if namespace.config is not None:
        loader = config_loader or (lambda p: yaml.safe_load(open(p)))
        base = deepcopy(loader(namespace.config))

    # Merge CLI overrides (non-None values only).
