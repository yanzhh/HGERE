"""Tests for train_span_classifier refactoring.
...
from hgere.commands.train_span_classifier import _config_to_namespace
...
def _patch_main(fake_run: Any) -> Any:
    ...
    stack.enter_context(
        patch(
            "hgere.commands.train_span_classifier.run_train_span_classifier",
...
class TestMainRouting:
    def test_positional_arg_loads_config(self, tmp_path: Path) -> None:
        ...
        with _patch_main(lambda args: captured.append(args)):
            from hgere.commands.train_span_classifier import main
    ...
    def test_config_flag_loads_config(self, tmp_path: Path) -> None:
        ...
        assert len(captured) == 1
        assert captured[0].model_dir.endswith("model")
    ...
    def test_config_flag_with_override(self, tmp_path: Path) -> None:
        ...
        with _patch_main(lambda args: captured.append(args)):
            ...
            main(["--config", config_path, "--train_params__learning_rate", "3e-6"])

        assert captured[0].learning_rate == pytest.approx(3e-6)
