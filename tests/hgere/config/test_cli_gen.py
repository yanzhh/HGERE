...
from hgere.config.cli_gen import (
    SEP,
...
class TestBuildArgparser:
    def test_config_flag_always_present(self) -> None:
        parser = build_argparser(Outer)
        action_dests = {a.dest for a in parser._actions}
        assert "config" in action_dests
...
class TestNamespaceToNestedDict:
    def test_flat_keys_stay_flat(self) -> None:
        ns = argparse.Namespace(name="foo", count=3, config=None)
        result = namespace_to_nested_dict(ns)
        assert result == {"name": "foo", "count": 3}

    def test_prefixed_key_becomes_nested(self) -> None:
        ns = argparse.Namespace(train_params__learning_rate=1e-5, config=None)
        result = namespace_to_nested_dict(ns)
        assert result == {"train_params": {"learning_rate": 1e-5}}

    def test_none_values_excluded(self) -> None:
        ns = argparse.Namespace(name="foo", count=None, config=None)
        result = namespace_to_nested_dict(ns)
        assert "count" not in result

    def test_config_key_excluded(self) -> None:
        ns = argparse.Namespace(config="path.yaml", name="foo")
        result = namespace_to_nested_dict(ns)
...
class TestApplyConfigAndCli:
    ...
    def test_cli_overrides_config_file(self, tmp_path: Path) -> None:
        ...
        ns = argparse.Namespace(
            config=config_path,
            train_params__learning_rate=2e-6,  # override
        )
        result = apply_config_and_cli(ns, PrunerTrainConfig)
        assert result.train_params.learning_rate == 2e-6

    def test_pydantic_defaults_fill_missing_fields(self, tmp_path: Path) -> None:
        config_path = self._write_yaml(
            tmp_path,
            {
                "label_set": "gsap",
    ...
    def test_invalid_config_raises(self, tmp_path: Path) -> None:
        config_path = self._write_yaml(
            tmp_path,
            {"label_set": "gsap"},  # missing required fields
        )
        ns = argparse.Namespace(config=config_path)
