...
class Sent_item:
    ...
    def build(self):
        ...
        self.obj_subtoken_pos = (
            obj_subtoken_positions  # e.g.: [(2, 2), (17, 18), (25, 30)]
        )
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(len(subword_sentence_boundaries) - 1):  # n for sentence
                ...
                for start, end, label in std_ner:
                    std_entity_labels[(start, end)] = (
                        label  # std_entity_labels: record gold entities of current sentence, e.g.: {(4, 7): 'Task', (13, 14): 'Method', (17, 18): 'Material', (22, 23): 'Material', (34, 36): 'Method', (38, 39): 'Method'}
                    )
                    self.ner_golden_labels.add(
                        ((l_idx, n), (start, end), label)
                    )  # add (doc_id, sent_id) into gold entities.     e.g.: {((0, 0), (13, 14), 'Method'), ((0, 0), (22, 23), 'Material'), ((0, 0), (17, 18), 'Material'), ((0, 0), (34, 36), 'Method'), ((0, 0), (38, 39), 'Method'), ((0, 0), (4, 7), 'Task')}

                self.global_predicted_ners[(l_idx, n)] = list(
                    sentence_ners
                ...
                entities = list(
                    sentence_ners
                )  # [[4, 5, 'NIL'], [4, 7, 'Task'], [13, 13, 'NIL'], [13, 14, 'Method']]
                ...
                for x in sentence_relations:  # example of x: [11, 13, 5, 6, 'USED-FOR']
                    ...
                    if w not in pos2label:  # no reverse relation of x in pos2label.
                        ...
                        if (
                            x[4] in self.sym_labels[1:]
                        ):  # if relation of x is symmetric, add reverse relation of x into pos2label.
                            pos2label[w] = label_map[
                                x[4]
                            ]  # pos2label[w] get the label_id for the reverse relation of x;
                        else:
                            # if not self.args.no_pseudo_label:
                            pos2label[w] = (
                                label_map[x[4]] + len(label_map) - len(self.sym_labels)
                ...
                for sub in entities:  # sub: iterated entity of a sentence  as subject.  e.g.: [48, 48, 'Generic']                 # only one subject for an input sequence
                    ...
                    if sub[0] == len(token2subword):
                        continue
                    sub_s = (
                        token2subword[sub[0]] - doc_offset + 1
                    )  # entity head id in input sequence
                    sub_e = (
                        token2subword[sub[1] + 1] - doc_offset
                    )  # entity tail id in input sequence
                    # sub_label = ner_label_map[sub[2]]                                                   # predicted subject label
                    sub_label_std = ner_label_map.get(
                        std_entity_labels.get((sub[0], sub[1]), "NIL")
                    )
                    ...
                    if self.use_typemarker:
                        ...
                        r_m = "[unused%d]" % (
                            2 + sub_label_std + len(self.ner_label_list)
                        )
                    else:
                    ...
                    )  # add typemarker into input subtokens
                    sub_e += 2
                    # else:
                    ...
                    for (
                        ...
                        obj_label,
                    ) in sentence_ners:  # add objects from predicted ners
